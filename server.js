...
app.put('/api/produtos/:id', async (req, res) => {
    ...
    const fields = Object.keys(patch).map((field, index) => `"${field}" = $${index + 1}`);
    ...
    // O ID será o último parâmetro
    const sql = `UPDATE produtos SET ${fields.join(', ')}, "atualizadoEm" = $${fields.length + 1} WHERE id = $${fields.length + 2}`;
